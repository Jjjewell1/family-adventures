...
Process uploaded logo:
  ...
  2. Generate <prefix>.png (512x512) for nav/footer
  3. Generate <prefix_fav>.png (64x64) for browser tab
  4. Generate <prefix_og>.png (1200x630) with logo centered on gradient background

Usage: python process-logo.py <input_path> <output_prefix>
  e.g. python process-logo.py /tmp/upload.jpg /app/build/client/uploads/branding/logo-1721952000000
"""
import sys
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python process-logo.py <input_path> <output_prefix>")
        print("  e.g. python process-logo.py /tmp/upload.jpg /app/build/client/uploads/branding/logo-1721952000000")
        sys.exit(1)
    ...
    logo_path = output_prefix.with_suffix('.png')
    logo.save(logo_path, "PNG")
    print(f"Saved logo: {logo_path}")

    # Generate favicon (64x64) — uses timestamp from parent filename
    ts = output_prefix.name.replace('logo-', '')
    fav_path = output_prefix.parent / f"favicon-{ts}.png"
    favicon = make_square(processed, 64, padding_pct=0.05)
    favicon.save(fav_path, "PNG")
    print(f"Saved favicon: {fav_path}")

    # Generate OG image (1200x630)
    og_path = output_prefix.parent / f"og-{ts}.png"
