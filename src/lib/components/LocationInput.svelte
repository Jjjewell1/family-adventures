    ...
    <div class="flex-1 relative">
      ...
      {#if showDropdown && results.length > 0}
        <div
          ...
          class="absolute z-50 mt-1 w-full rounded-lg border border-cream-200 bg-white shadow-lg max-h-60 overflow-auto dark:bg-ink-700 dark:border-ink-600"
        >
          {#each results as result}
            <button
              type="button"
              class="w-full text-left px-4 py-3 hover:bg-forest-50 transition-colors border-b border-cream-100 last:border-0 dark:hover:bg-ink-600 dark:border-ink-600"
